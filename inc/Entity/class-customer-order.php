 ...
 * @package    OU
 * @subpackage OU/subscriber
 */

namespace DKO\OU\Entity;
...
class Customer_Order {
	...
	const OU_META_KEY_ORDER_NUM_SEED    = '_order_num';
	const OU_META_KEY_DISPLAY_ORDER_NUM = '_displayed_order_num';
	const OU_TRANSIENT_KEY              = 'wc_order_utils_free_num';
	...
	public function get_order_num(): string {
		...
		return $this->get_meta_with_default( self::OU_META_KEY_DISPLAY_ORDER_NUM, false );
	}
	...
	public function get_order_num_seed(): string {
		...
		$custom_num = $this->get_meta_with_default( self::OU_META_KEY_ORDER_NUM_SEED, false );
		if ( empty( $custom_num ) ) {
	...
	public static function get_next_order_num(): int {
		...
		$custom_num = get_transient( self::OU_TRANSIENT_KEY );
		...
		if ( false === $custom_num ) {
			...
			$orders     = wc_get_orders(
				array(
					'meta_query' => array(
						array(
							'key'     => self::OU_META_KEY_ORDER_NUM_SEED,
							'compare' => 'EXISTS',
						),
					),
					'date_query' => array(
			...
			set_transient( self::OU_TRANSIENT_KEY, $custom_num, DAY_IN_SEOUDS );
		} else {
			++$custom_num;
			set_transient( self::OU_TRANSIENT_KEY, $custom_num, DAY_IN_SEOUDS );
		}
	...
	public function reset_order_num() {
		$this->remove_meta( self::OU_META_KEY_ORDER_NUM_SEED );
	}
	...
	public function set_order_number( string $order_num ) {
		$this->store[ self::OU_META_KEY_DISPLAY_ORDER_NUM ] = $order_num;
	}
	...
	public function set_order_num_seed( string $order_num_seed ) {
		$this->store[ self::OU_META_KEY_ORDER_NUM_SEED ]    = $order_num_seed;
		$this->store[ self::OU_META_KEY_DISPLAY_ORDER_NUM ] = $this->construct_order_num();
	}
